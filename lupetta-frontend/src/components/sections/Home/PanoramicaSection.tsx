...
export default function PanoramicaSection() {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
      <RevealSection className="text-center mb-12 sm:mb-16 lg:mb-20" animation="sr-reveal-up">
        <span className="text-[#006071] font-bold text-sm tracking-widest uppercase">Panoramica</span>
        <h2 className="text-4xl sm:text-5xl md:text-6xl text-[#006071] tracking-tight mt-4 mb-8">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {CARDS.map((card, i) => (
          <TiltCard3D key={i} className="rounded-3xl">
            <RevealSection className="bg-white p-5 sm:p-6 lg:p-8 rounded-3xl shadow-sm border border-slate-100 group" animation="sr-flip-in" delay={`sr-delay-${(i + 1) * 2}`}>
            <div className="w-16 h-16 bg-[#65b32e]/10 rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500">
              <svg className="w-8 h-8 text-[#65b32e]" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d={card.iconPath} />
              </svg>
            </div>
            <h3 className="text-2xl text-slate-900 mb-4 tracking-tight">{card.title}</h3>
            <p className="text-slate-500 text-base leading-relaxed">{card.desc}</p>
            <div className="mt-8 h-1 w-12 rounded-full bg-[#65b32e] group-hover:w-20 transition-all duration-500" />
            </RevealSection>
          </TiltCard3D>
        ))}
      </div>
