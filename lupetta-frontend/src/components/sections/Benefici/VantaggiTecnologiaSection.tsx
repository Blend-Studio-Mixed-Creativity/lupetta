    ...
    <section className="bg-white border-y border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 lg:mb-24 animate-slide-up">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-slate-900 tracking-tight font-semibold leading-tight">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-stretch">
          ...
          <div className="lg:col-span-5 relative w-full h-full min-h-[400px] animate-slide-right group">
            <div className="absolute inset-0 bg-gradient-to-b from-[#0b1a20] to-[#006071] rounded-[2rem] shadow-[0_20px_50px_rgb(0,96,113,0.3)] p-1 overflow-hidden">
              ...
              <div className="relative h-full w-full bg-[#0b1a20]/90 backdrop-blur-2xl rounded-[1.8rem] p-8 sm:p-10 flex flex-col items-center justify-center border border-white/10">
                <h3 className="text-xl sm:text-2xl font-semibold text-white mb-8 text-center mt-4">Statistiche rilevate</h3>
                <div className="w-full space-y-4 sm:space-y-6">
                  {STATS.map((stat, i) => (
                    <div key={i} className="group/stat flex items-center justify-between bg-white/5 hover:bg-white/10 border border-white/5 hover:border-[#65b32e]/30 px-6 sm:px-8 py-5 sm:py-6 rounded-2xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-[0_10px_30px_rgb(101,179,46,0.15)]">
                      <span className="text-slate-300 font-medium text-sm sm:text-base group-hover/stat:text-white transition-colors">{stat.label}</span>
                      <span className="text-[#65b32e] font-bold text-2xl sm:text-3xl tracking-tight drop-shadow-[0_0_15px_rgba(101,179,46,0.4)]">{stat.value}</span>
                    </div>
                  ))}
                </div>
              </div>
